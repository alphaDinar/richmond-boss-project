...
const EditProduct = ({ searchParams }: { searchParams: { obj: string } }) => {
  const product = JSON.parse(searchParams.obj) as Product;
  ...
  const [displayName, setDisplayName] = useState<Product['displayName']>(product.displayName);
  ...
  const [topFeatures, setTopFeatures] = useState<Product['topFeatures']>(product.topFeatures);

  const [image, setImage] = useState<Product['image']['url']>(product.image.url);
  const [imageFile, setImageFile] = useState<File | null>(null);

  const [coverImage, setCoverImage] = useState<Product['coverImage']['url']>(product.coverImage.url);
  const [coverImageFile, setCoverImageFile] = useState<File | null>(null);

  const [gallery, setGallery] = useState<Product['gallery']>(product.gallery);
  const [galleryFiles, setGalleryFiles] = useState<(File | null)[]>([]);

  const [bundles, setBundles] = useState<Product['bundles']>(product.bundles);
  const [variations, setVariations] = useState<Product['variations']>(product.variations);
  ...
  const editProduct = async () => {
    setFormLoading(true);
    const edit = async (finalImage: MediaType, finalCoverImage: MediaType, finalGallery: MediaType[]) => {
      // const stamp = new Date().getTime();
      const finalProduct: Omit<Product, 'id' | 'reviews' | 'timestamp'> = {
        category: category,
        brand: brand,
        name: name,
      ...
      await updateDoc(doc(fireStoreDB, 'Products/' + product.id), finalProduct)
      const productListTemp = productList.filter((prod) => prod.id !== product.id);
      const prodTemp: Product = { ...finalProduct, id: product.id, reviews: product.reviews, timestamp: product.timestamp };
      const productListTempEdited = [...productListTemp, prodTemp];
      updateDoc(doc(fireStoreDB, 'Register/' + 'products'), {
        list: productListTempEdited
      })
    ...
    const uploadMediaSet = async () => {
      const finalImage = imageFile ? createMediaType(imageFile, (await uploadImage(imageFile))) : product.image;
      const finalCoverImage = coverImageFile ? createMediaType(coverImageFile, (await uploadImage(coverImageFile))) : product.coverImage;
      let finalGallery = [...gallery.filter((el) => (el.url.split(':')[0] != 'blob'))];
      ...
      edit(finalImage, finalCoverImage, finalGallery);
    }
  ...
  const deleteProduct = () => {
    const ans = confirm(`Are you sure you want to delete ${product.name}`);
  ...
  return (
    ...
    <form id="hor" className="grid gap-4 py-10" onSubmit={(e) => { e.preventDefault(); editProduct(); }}>
      <Button color="warning" onClick={() => router.back()}>Back</Button>
      <Button color="danger" onClick={deleteProduct}>Delete Product</Button>
      <div>
        <span>name</span>
      ...
      <div>
        ...
        <SelectBox items={categories} setItem={setCategory} defKey={category} />
      </div>

      <div>
        <span>Brand {brand}</span>
        <SelectBox items={brands} setItem={setBrand} defKey={brand} />
      </div>
      <div>
        ...
        <div>
          ...
          <ul className="list-disc pl-5">
            {bundles.map((bun, index) => (
              <div className="border p-1 flex justify-between" key={index}>
                <li>
